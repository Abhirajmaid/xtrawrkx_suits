...
export default function QuickActionsWidget() {
  ...
  const fetchCounts = async () => {
    try {
      ...
      const [dealsResponse, contactsResponse, companiesResponse] = await Promise.allSettled([
        dealService.getAll({ pagination: { pageSize: 1 } }),
        contactService.getAll({ pagination: { pageSize: 1 } }),
        leadCompanyService.getAll({ pagination: { pageSize: 1 } }),
      ]);

      // Extract total counts from meta pagination or response length
      const dealsCount = dealsResponse.status === 'fulfilled' 
        ? (dealsResponse.value?.meta?.pagination?.total || dealsResponse.value?.data?.length || 0)
        : 0;
      
      const contactsCount = contactsResponse.status === 'fulfilled'
        ? (contactsResponse.value?.meta?.pagination?.total || contactsResponse.value?.data?.length || 0)
        : 0;
      
      const companiesCount = companiesResponse.status === 'fulfilled'
        ? (companiesResponse.value?.meta?.pagination?.total || companiesResponse.value?.data?.length || 0)
        : 0;
      ...
      setCounts({
        ...
        reports: 0, // Reports would need a separate service
      });
    } catch (error) {
      console.error("Error fetching quick access counts:", error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div>
        ...
        <div className="space-y-3">
          {[
            ...
            { name: 'Proposal Template', type: 'Document Template' }
          ].map((template, index) => (
            <button
              key={index}
